...
/*   Updated: 2021/02/22 11:12:38 by mvankoek         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_view(t_rush *ptr, int x, int y) //check view coord correspond a la view calculer
{
...
int	ft_check_allowed(t_rush *ptr, int x, int y)
{
	...
	if (c == 0 || c > ptr->size)
		return (0);
	while (y2 < ptr->size) //check colonne si nbr diff
	{
	...
	x2 = 0;
	y2 = y;
	while (x2 < ptr->size) //check couloir si nbr diff
	{
